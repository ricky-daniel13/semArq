...
def gen():
    ...
    for a in range(0, 12):
        ...
        shan = shant()
        fun = funct(func)
        mem.write(str(op) + str(Rs)[0:2] + " //opcode + 2 bits de rs \n")
        mem.write(str(Rs)[2:5] + str(Rt) + " //rs " + str(rS) + " rt " + str(rT) + " \n")
        mem.write(str(Rd) + str(shan)[0:3] + " //rd " + str(rD) + " + 3 bits de shant \n")
        mem.write(str(shan)[3:5] + str(fun) + " //shant + //funct - " + func + "\n")
    mem.close()
